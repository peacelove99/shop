...
(
   ...
   u_id                 int comment '用户实体的主键',
   a_name               varchar(30) comment '地址的收件人',
   a_phone              varchar(14) comment '收件人电话',
   a_detail             varchar(200) comment '收件人详细地址',
   a_state              int comment '是否是默认地址0不是1是',
   primary key (a_id)
);

alter table address comment '用户个人的地址实体，用户存储地址信息';
...
alter table cart comment '购物车模块的购物车实体';
...
(
   ...
   o_id                 varchar(64) comment '订单编号是字符串类别也是唯一标识符',
   p_id                 int comment '商品的唯一主键',
...
create table orders
(
   o_id                 varchar(64) not null comment '订单编号是字符串类别也是唯一标识符',
   u_id                 int comment '用户实体的主键',
   a_id                 int comment '地址实体的唯一主键',
   o_count              decimal(12,2) comment '订单的总金额',
   o_time               datetime comment '订单的详细时间',
   o_state              int comment '订单状态0未付款，1付块待发货，2发货待收货，3收货待评价，4订单完成，5退货',
   primary key (o_id)
);
...
create table user
(
   u_id                 int not null auto_increment comment '用户实体的主键',
   u_name               varchar(20) not null comment '用户账号',
   u_password           varchar(64) not null comment '用户密码',
   u_email              varchar(50) not null comment '用户的邮箱',
   u_sex                varchar(4) comment '用户性别',
   u_status             int comment '用户的激活状态',
